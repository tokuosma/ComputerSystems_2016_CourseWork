...
//	FILL OUT THIS DATA STRUCTURE TO ASK DATA
    ...
    if (I2C_transfer(*i2c, &i2cTransaction)) {
    	...
    	;
//    	System_printf("Reg: %08x %08x %08x\n", rxBuffer[2],rxBuffer[1],rxBuffer[0]);
//    	System_flush();
    	uint32_t pressure = (((rxBuffer[0] << 16) | (rxBuffer[1] << 8 ) | (rxBuffer[2])) >> 4 )  ;
    	uint32_t temperature = (((rxBuffer[3] << 16) | (rxBuffer[4] << 8 ) | (rxBuffer[5])) >> 4 )  ;
    	*temp = ((bmp280_convert_temp(temperature) -32) * ((double)5/9));
		*pres = (bmp280_convert_pres(pressure) / 100);
